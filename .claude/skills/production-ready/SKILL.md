...
6. **Registry sync**
   ...
   - **Registry dependency completeness**: Read the component's imports in `registry/ui/<name>.tsx` and verify every import path has a matching entry in `registryDependencies` in `registry/<name>.json`. Missing dependencies cause install-time failures for framework consumers
...
11. **Cross-environment safety**
   - Verify the component uses only OpenTUI intrinsics (`<box>`, `<text>`, `<span>`, etc.) and framework hooks (`useTheme`, `textStyle`). Flag any direct use of `document`, `window`, browser DOM APIs, or terminal-specific APIs (e.g., raw ANSI codes). Components distributed via the registry must render in both terminal and browser
...
12. **Test coverage by code path**
    ...
    - **Polymorphic prop shapes**: When a prop accepts `ReactNode` or a union type, verify tests exercise it with different value shapes (string, JSX element, `null`). String-only tests miss rendering issues with nested elements

13. **Documentation — code examples are valid**
...
15. **Documentation — API table and controls**
...
16. **Documentation — example coverage**
...
17. **Demo validity and feature coverage**
