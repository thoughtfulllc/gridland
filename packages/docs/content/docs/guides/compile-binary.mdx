...
Bun's `--bytecode` flag is not compatible with Gridland CLIs. The `@opentui/core` package uses top-level `await` internally, which bytecode compilation does not support.
</Callout>

## Cross-compilation

Bun supports cross-compiling to other platforms with the `--target` flag. However, `@opentui/core` includes platform-specific native bindings that are resolved at build time, so **you must build on the target platform** (or in a CI runner matching the target OS/arch).
...
If your CLI does not depend on `@opentui/core`'s native bindings (e.g. a pure-JS tool using only `@gridland/core` utilities), cross-compilation works with `--target`:
