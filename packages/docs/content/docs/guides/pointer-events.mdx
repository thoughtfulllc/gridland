...
Combine `onMouseOver` with `useFocus().focus()` to make hovering an element claim keyboard focus.
This is the canonical way to make a multi-modal UI (keyboard + mouse) where hovering "previews"
the focused item.
...
function Item({ id }: { id: string }) {
  const { focus, focusRef } = useFocus({ id })
  return (
...
- [Focus and Navigation](/docs/guides/focus-and-navigation) — how keyboard focus interacts with
  pointer events.
