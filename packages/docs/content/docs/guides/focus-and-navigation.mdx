...
```tsx title="app.tsx"
import { FocusProvider, useFocus, useKeyboard, useCapturedKeyboard, useShortcuts, useFocusedShortcuts, FocusScope } from "@gridland/utils"
import { GridlandProvider, StatusBar, useFocusBorderStyle } from "@gridland/ui"
...
**`useCapturedKeyboard(focusId)`** captures keyboard events for a selected focusable element and returns a `capture` function. Pass it as the `useKeyboard` prop to a child component (e.g., `PromptInput`) so the child's key handler fires only when the parent is selected.
