...
selection-scoped keyboard routing, shortcut hints, and theme-aware focus borders.
Interactive components call it directly; the underlying primitives remain available
for edge cases.

```tsx title="app.tsx"
import { GridlandProvider, StatusBar, useInteractive } from "@gridland/ui"
...
function Cell({ focusId, label, autoFocus }) {
  ...
  // 3. Handle keys while the component is selected
  interactive.onKey((e) => {
...
// 4. Display context-sensitive shortcuts from the currently focused component
...
`onKey(handler)`, `focus()`, `blur()`, `select()`, `deselect()`, `borderColor`,
`borderStyle`.
