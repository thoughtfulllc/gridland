  ...
  return (
    <CommandProvider>
      <box flexDirection="column" flexGrow={1}>
              ...
              <Message key={msg.id} role={msg.role} isStreaming={msgStreaming}>
                <Message.Content>
                  {msg.parts?.map((part, j) => {
                    switch (part.type) {
                      case "text":
                        return <Message.Text key={j} isLast={j === msg.parts.length - 1 && msgStreaming}>{part.text}</Message.Text>
                      case "reasoning":
                        return <Message.Reasoning key={j} />
                      default:
                        return null
                    }
                  })}
        ...
        {/* Prompt input with focus management */}
        <PromptInput
...
| [`CommandProvider`](/docs/components/command-provider) | Registers `/model` and `/clear` slash commands |
