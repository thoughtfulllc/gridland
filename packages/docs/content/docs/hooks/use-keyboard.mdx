...
<Callout type="info">
For focus-aware components, prefer [`useInteractive({ id }).onKey(handler)`](/docs/hooks/use-interactive#onkey)
...
[Focus](/docs/interaction/focus) for the full model. Reach for `useKeyboard`
...
[`useInteractive({ id }).onKey(handler)`](/docs/hooks/use-interactive#onkey)
...
```tsx title="Focus-scoped: only fires when this id is focused"
