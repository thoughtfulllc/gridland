...
The `onSubmit` signature matches Vercel AI SDK's `sendMessage({ text })`, so you
can pass it through directly.

```tsx title="AI SDK integration"
...
| `onSubmit` | `(message: { text: string }) => void \| Promise<void>` | - | Called on submit. Matches AI SDK `sendMessage({ text })`. |
