...
OpenTUI Web uses JSX intrinsic elements that map to renderable components.
These are used inside `<OpenTuiCanvas>`.
...
canvas cell grid, not to an HTML `<div>`. They only work inside an
`<OpenTuiCanvas>` wrapper.
</Callout>
