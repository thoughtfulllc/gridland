...
```bash title="Terminal"
bunx create-gridland add modal
```
...
Modal listens for Escape via `useKeyboard` from `@gridland/utils` internally
no keyboard plumbing is required from the caller. Render it anywhere inside
a `GridlandProvider` or `FocusProvider` and the Escape handler will fire.
