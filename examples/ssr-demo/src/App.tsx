...
import { BrowserRenderer } from "../../../packages/web/src/browser-renderer"
import { bufferToText } from "../../../packages/web/src/buffer-to-text"

function bufferToHtml(buffer: { char: Uint32Array; fg: Float32Array; bg: Float32Array; width: number; height: number }): string {
  const lines: string[] = []
  ...
  for (let row = 0; row < buffer.height; row++) {
    ...
    for (let col = 0; col < buffer.width; col++) {
      ...
      const cellBg = ba > 0 ? `rgba(${br},${bg},${bb},${ba.toFixed(2)})` : "rgba(0,0,0,0)"
  ...
  return (
    <box flexDirection="column" width="100%" height="100%" padding={1}>
      ...
      >
        ...
        <text fg="#b48ead">Toggle the button below to see what an agent sees.</text>
      </box>
      ...
      <box marginTop={1} border={true} borderStyle="rounded" borderColor="#a3be8c" padding={1} flexDirection="column">
        <text fg="#ebcb8b" bold={true}>About this demo</text>
        <text fg="#d8dee9">Click the "Agent View" toggle in the bottom-right to see</text>
        <text fg="#d8dee9">how this page looks as raw Unicode characters — the same</text>
        <text fg="#d8dee9">output that an LLM or screen reader would receive.</text>
      </box>
    </box>
  )
}
...
export function App() {
  ...
  // When toggled on, snapshot the buffer and keep it updated
  useEffect(() => {
    if (showText && rendererRef.current) {
      const snap = () => {
        const buf = rendererRef.current!.buffer
        setTextContent(bufferToText(buf))
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", position: "relative", background: "#1e1e2e" }}>
      {/* Canvas TUI layer */}
      <div style={{ width: "100%", height: "100%", visibility: showText ? "hidden" : "visible" }}>
        <TUI style={{ width: "100%", height: "100%" }} onReady={handleReady}>
          <GridlandDemo />
        </TUI>
      </div>
      ...
      <button
        ...
        style={{
          ...
          padding: "10px 18px",
          borderRadius: "8px",
          border: showText ? "2px solid #a3be8c" : "2px solid #5e81ac",
      ...
      >
        <span style={{ fontSize: "16px" }}>{showText ? "\u{1F441}" : "\u{1F916}"}</span>
